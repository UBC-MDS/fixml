...
To evaluate if newer LLMs improve performance, we obtained outputs from `gpt-4o` and `gpt-4-turbo` on the `lightfm` repository and plotted the graph to show how our tool performs in terms of accuracy and consistency when switched the LLMs. 

By comparing with the results from `gpt-3.5-turbo` (shown in [@fig-accu-mean-sd-plot]), we observed an increase in consistency using newer LLMs given the smaller standard deviations. However, we found that `gpt-4o` returned "Satisfied" for all items, while `gpt-4-turbo` deviated more from the ground truth for item `3.5 Check for Duplicate Records in Data`, `5.3 Ensure Model Output Shape Aligns with Expectation` when compared to `gpt-3.5-turbo`.

```{python}
#| label: fig-llm-mean-sd-plot
#| fig-cap: Analysis of the scores per checklist item on repository `lightfm` using different GPT versions . Black dot and line represents the mean and standard deviation of scores from the tool. Green diamond represents the ground truth score. 
...
Based on the graph, there could be a potential improvement of our tool in consistency and/or accuracy when switching to newer LLMs. The caveat is, what in our package works well for the current LLM might not necessarily well for the newer LLMs. This implies exploration in different structure, e.g. prompt engineering for `gpt-4-turbo`, etc.
...
Cost affiliated with LLM usage could be one consideration for users to use our tool. Future improvements involve sharing our cost data and the calculation of estimated cost (e.g. cost against number of line of code), while facilitates users to estimate costs and carry out their cost-benefit analysis to make informed decisions when using our tool.
